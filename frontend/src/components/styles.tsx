...
export const StyledListItemText = styled(ListItemTextS)`
  padding-left: 5px;
  padding-right: 5px;
  text-decoration: ${(props: { strikethrough?: boolean }) =>
    props.strikethrough ? "line-through" : undefined};
`;

const ListItemS: SFC<ListItemProps> = ({ children, className, ...props }) => (
...
const ListItemCheckboxS: SFC<CheckboxProps> = ({ className, ...props }) => (
...
const PaperS: SFC<PaperProps> = ({ children, className, ...props }) => (
...
const TextFieldS: SFC<TextFieldProps> = ({ className, ...props }) => (
